...
interface TradingPanelProps {
  ...
  currentPrice: number;
  symbol: string;
  onOrder: (side: 'long' | 'short', leverage: number, amount: number, price: number, tp?: number, sl?: number, isLimit?: boolean, reason?: string) => void;
}
    ...
    <Card className="bg-zinc-950 border-zinc-800 text-zinc-100">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Slider
            value={[leverage]}
            onValueChange={(v: any) => setLeverage(Array.isArray(v) ? v[0] : v)}
            max={100}
            min={1}
            step={1}
            className="py-2"
        ...
        <div className="space-y-1 pt-1">
          <label className="text-[10px] text-zinc-400">📝 交易理由 *</label>
          <textarea value={reason} onChange={e => setReason(e.target.value)} rows={2}
            placeholder="写下你的交易理由..."
            className="w-full px-2 py-1.5 bg-zinc-900 border border-zinc-800 rounded text-xs text-zinc-100 outline-none resize-none focus:border-zinc-600" />
        </div>

        <div className="grid grid-cols-2 gap-3 pt-2">
          <Button
            onClick={() => { onOrder(
              'long', leverage, amount,
              orderType === 'market' ? currentPrice : limitPrice,
              tp ? Number(tp) : undefined, sl ? Number(sl) : undefined,
              orderType === 'limit', reason || undefined
            ); setReason(''); }}
            disabled={!reason.trim()}
            className="bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-2 h-10 font-bold disabled:opacity-50"
          >
          ...
          <Button
            onClick={() => { onOrder(
              'short', leverage, amount,
              orderType === 'market' ? currentPrice : limitPrice,
              tp ? Number(tp) : undefined, sl ? Number(sl) : undefined,
              orderType === 'limit', reason || undefined
            ); setReason(''); }}
            disabled={!reason.trim()}
            className="bg-rose-600 hover:bg-rose-700 text-white flex items-center gap-2 h-10 font-bold disabled:opacity-50"
          >
