...
import { Position, TradeHistory, PendingOrder } from '@/lib/tracker/types';
import { XCircle, History, LayoutList, Clock, Cpu } from 'lucide-react';
import { formatPrice, formatPercent } from '@/lib/tracker/utils';
                  ...
                  <div key={pos.id} className="p-3 rounded-lg bg-zinc-900 border border-zinc-800">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        {pos.reason && (
                          <div className="mt-1 text-[9px] text-zinc-500 border-l-2 border-blue-500/40 pl-1.5 line-clamp-1" title={pos.reason}>
                            💬 {pos.reason}
                          </div>
                        )}
                      </div>
              ...
              <div className="space-y-2">
                {history.map((h) => (
                  <div key={h.id} className="p-2 rounded bg-zinc-900/30 border border-zinc-800/50">
                    ...
                    {/* Trade Reason */}
                    {h.reason && (
                      <div className="mt-1.5 text-[9px] text-zinc-500 border-l-2 border-blue-500/40 pl-1.5">
                        💬 {h.reason}
                      </div>
                    )}
